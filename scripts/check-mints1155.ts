...
import { handleGeneralMints1155Single } from "../src/handlers/general-mints1155";

async function main() {
  const { context, event } = TestHelpers.GeneralMints1155.TransferSingle.mock({
    params: {
  ...
  await handleGeneralMints1155Single({ event, context });
